...
class MAESTRO(PianoRollAudioDataset):
    ...
    def files(self, group):
        if group not in self.available_groups():
            ...
            files = sorted([(os.path.join('./', metadata['audio_filename'][row].replace('.wav', '.flac')),  os.path.join('./', metadata['midi_filename'][row])) for row in metadata['split'].keys() if metadata['split'][row] == group])
